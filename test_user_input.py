...
def test_player_name_char_limit(monkeypatch):
    ...
    assert user_input.player_name("prompt", char_limit=CHAR_LIMIT) == ('w'*CHAR_LIMIT)
...
bankroll_value_error_input_sequence = ['one hundred', '$100', '100.00', str(BANKROLL_MIN+1)]
...
@pytest.mark.parametrize(
    ...
    [(bankroll_low_input_sequence, BANKROLL_MIN), (bankroll_high_input_sequence, BANKROLL_MAX)],
)
def test_bankroll_min_max_branch(monkeypatch, input_list, expected_output):
...
pass_line_value_error_input_sequence = ['one hundred', '$100', '100.00', str(PASS_LINE_MIN+1)]
...
@pytest.mark.parametrize(
    ...
    [(pass_line_low_inputs, PASS_LINE_MIN), (pass_line_high_inputs, PASS_LINE_MAX)]
)
def test_pass_line_min_max_branch(monkeypatch, input_list, expected_outcome):
...
def test_come_out_prompt_input_branch(monkeypatch, string, expected_output):

    def return_string(text: str):
        return string
    monkeypatch.setattr(user_input, "input", return_string, raising=False)
...
def test_roll_again_prompt_input_branch(monkeypatch, string, expected_output):
